...
export const editCategory = async (categoryId: number,category:Category): Promise<CategoryResponse> => {
  const response = await apiClient.put(`/admin/edit-category/${categoryId}`, category);
  return response.data;
};
