...
const saveEditComment = async (content: string) => {
  try {
    const updatedContent = await editComment(props.comment.id!, {
      ...props.comment,
      content,
    })
...
<template>
  <div class="mb-4 rounded p-3 text-white">
    ...
    <div v-else>
      <div v-if="!isEditing" class="mb-4">{{ props.comment.content }}</div>
    ...
    <button
      ...
      class="mt-2 text-sm cursor-pointer bg-publish-button hover:bg-publish-button-hover text-white rounded px-2 py-1"
    >
    ...
    >
      <CommentWithReplies
        v-for="child in props.comment.childComments"
        :key="child.id"
        :comment="child"
        @reply-add="emit('reply-add', $event, props.comment.id!)"
      />
    </div>
  </div>
</template>
