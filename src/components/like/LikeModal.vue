...
<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="isModalOpen"
        class="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-xs"
        @click.self="emit('close')"
      >
        <div
          class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 max-h-[70vh] overflow-auto"
        >
          <h2 class="text-2xl font-semibold mb-4 text-center">
            Personnes qui ont liké la conversation
          </h2>
          <ul class="mb-4">
          ...
          <button
            @click="emit('close')"
            type="button"
            class="w-full py-2 bg-red-600 text-white rounded transition"
          >
            Fermer
          </button>
        </div>
