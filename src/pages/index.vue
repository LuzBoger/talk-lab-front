...
const loadConversations = async (title: string, categories: string[], page: number) => {
  try {
    isLoading.value = true
    const res = await searchPublicConversations(title, categories, page, convPerPage)

    
    publicConversations.value =
      res.conversations.map((conversation: any) => ({
        ...conversation,
...
watch(selectedCategories, async (newSelectedCategory) => {
  ...
  try {
    ...
    await loadConversations(searchQuery.value, newSelectedCategory, currentPage.value)
  } catch(error) {
...
watch(currentPage, async (newPage) => {
  try {
    ...
    await router.replace({query})
    await loadConversations(searchQuery.value,selectedCategories.value, newPage)
  } catch(erorr) {
    console.error('Erreru lors du chargement ')
...
onMounted(async() => {
  ...
  await loadConversations(searchQuery.value, selectedCategories.value, currentPage.value)
})
...
<template>
  <div>
    <header class="flex flex-col md:flex-row justify-between items-center m-4">
      <h1 class="text-xl text-white welcome-message-shadow">
        Bienvenue sur
    ...
    <section class="p-4 flex flex-col gap-4">
        ...
        <div v-if="!isLoading && publicConversations.length === 0" class="text-white text-center mt-6">
      ...
      <Pagination v-if="publicConversations.length > 0" v-model="currentPage" :total-items="totalConversations" :page-size="convPerPage" />
    </section>
  </div>
</template>
